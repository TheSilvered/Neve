...
void test_ctxCurMoveRightAscii(void) {
    Ctx ctx;
    ctxInit(&ctx, true);
    const char s[] = "abcd";
...
void test_ctxCurMoveRightMultiline(void) {
    Ctx ctx;
    ctxInit(&ctx, true);
    const char s[] = "ab\ncd\n";
    ctxAppend(&ctx, (UcdCh8 *)s, chArrLen(s));
...
void text_ctxCurMoveRightUnicode(void) {
    Ctx ctx;
    ctxInit(&ctx, true);
...
testList(
    testMake(test_ctxCurMoveRightAscii),
    testMake(test_ctxCurMoveRightMultiline),
    testMake(text_ctxCurMoveRightUnicode)
)
